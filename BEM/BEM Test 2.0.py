...
'''
================================================================================
UPDATE LOG 2.1 - Luke Jones
================================================================================


================================================================================
UPDATE LOG 2.0 - Luke Ryan
================================================================================

1) Added Convergence counter that prints out after each success Lines [359, 450-455]
    (Can create a file instead if print is too annoying)
2) Changed def polar_lookup Lines [151-208] to include a Viterna-Corrigan extrapolation during startup/stalling
...
def polar_lookup(polar, alpha_deg, AR=15.0):
    """
    Interpolate polar data at desired alpha.
    Standard interpolation between inputted data points, uses Viterna-Corrigan
    model extrapolation for high alpha.
    polar     : dict with polar["alpha"], polar["cl"], polar["cd"]
    alpha_deg : numpy array of requested angles of attack [deg]
    AR : Blade Aspect Ratio (15 for 500kW turbine)

    Returns:
      cl_i, cd_i : interpolated CL and CD arrays in data range, extrapolated outside data range
    """
    a = polar["alpha"]                              # polar alpha array [deg]
    cl = polar["cl"]                                # polar CL array [-]
    cd = polar["cd"]                                # polar CD array [-]

    # Find the last given values (stall point)
    alpha_s = a[-1]
    cl_s = cl[-1]
    cd_s = cd[-1]

    # Convert alpha stall point to radians
    alpha_s_rad = np.radians(alpha_s)

    # Approximate maximum drag coefficient at 90 degrees using Aspect Ratio
    ...
    # Lift constants
    a1 = b1 / 2.0
    a2 = (cl_s - cd_max * np.sin(alpha_s_rad) * np.cos(alpha_s_rad)) * (math.sin(alpha_s_rad) / math.cos(alpha_s_rad)**2)

    # Create output arrays
    cl_out = np.zeros_like(alpha_deg, dtype=float)
    cd_out = np.zeros_like(alpha_deg, dtype=float)

    for i, alpha in enumerate(alpha_deg):
        if alpha <= alpha_s:
            # Use standard interpolation for given data range
            # Clamps to left side if necessary
            cl_out[i] = np.interp(alpha, a, cl, left=cl[0])
            cd_out[i] = np.interp(alpha, a, cd, left=cd[0])
        else:
            # Apply Viterna-Corrigan formulas for high angles of attack
            # Clamp alpha to 90 degrees to prevent the formula from breaking
            alpha_clamped = min(alpha, 90.0) 
            a_rad = np.radians(alpha_clamped)
            
            # Viterna-Corrigan Drag
            cd_out[i] = b1 * math.sin(a_rad)**2 + b2 * math.cos(a_rad)
            
            # Viterna-Corrigan Lift
            cl_out[i] = a1 * math.sin(2 * a_rad) + a2 * (math.cos(a_rad)**2 / math.sin(a_rad))
...
def solve_bem_point(
    ...
    for i in range(n):
        ...
        for _ in range(itmax):
            ...
            ai, api = ai_next, api_next
            #Add 1 for no convergence
            n_fail += 1
        #Dispay the number of convergences for each point
        if n_fail > 0:
            print(f"  [Convergence counter] {n_fail} convergences for V0={V0:.1f} m/s")
